...
form the public key *g<sup>x</sup> mod p*.  For mathematical
...
To encrypt a message *M*, assumed to be an integer modulo *p*, draw a
random "ephemeral" exponent *y*, compute *Y = g<sup>y</sup> mod p*,
and form the ciphertext *(Y, M·Y mod p)*.
